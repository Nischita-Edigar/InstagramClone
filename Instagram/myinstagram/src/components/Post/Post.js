...
export default class Post extends Component {
    constructor(props){
      ...
      getComments=()=>{
        fetch("https://localhost:8080/comment/"+this.props.id)
      ...
      <div className='post_container'>
        {/* header */}
        {
          this.state.postList.map((item, index)=>(
            ...
            <Avatar  className="post_image" src={item.profileImage}/>
        ...
        {/* posts */}
        {
          this.state.postList.map((item, index)=>(
             <div>
                <img src={item.postImage}  alt="photograph" width="608px"  />
             </div>
          ))
        }        
        {/* analytics */}
        ...
        <div >
          {
           ...
           <input type="text" onKeyPress={this.submitComments} className='comment_box' placeholder='Add a comment...'/>
        </div>
      </div>
    )
  }
